...
1.1 Generating the Personal Genome (hap1.fa and hap2.fa)
```
rPGA personalize \ 'personalize' is a rPGA command to generate reference genome based known SNPs (VCF files) 
 -o /path/to/output/directory \ # output directory defined by user. Will be used in all following steps
...
```
1.2 Indexing and Mapping to hap1 and hap2 using STAR, and assigning haplotype specific reads
```
OUTPUT_BAM=/path/to/output/directory # defined by user in the last step

rPGA mapping \ 'mapping' is a command of rPGA package to run STAR to map input reads to two haplotype genomes
 -o $OUTPUT_BAM \ # output directory defined by user in the very first step. rPGA will search hap1.fa hap2.fa under this folder
